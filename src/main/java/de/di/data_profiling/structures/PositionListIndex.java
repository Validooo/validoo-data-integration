...
public class PositionListIndex {
    ...
    private List<IntArrayList> intersect(List<IntArrayList> clusters, int[] invertedClusters) {
        ...
        for (IntArrayList cluster : clusters) {
            Int2ObjectMap<IntArrayList> partialClusters = new Int2ObjectArrayMap<>();
            ...
            for (int recordIndex : cluster) {
                ...
                partialClusters.putIfAbsent(otherClusterIndex, new IntArrayList());
                partialClusters.get(otherClusterIndex).add(recordIndex);
            }

            for (IntArrayList partialCluster : partialClusters.values()) {
                if (partialCluster.size() > 1)
                    clustersIntersection.add(partialCluster);
            }
        }
